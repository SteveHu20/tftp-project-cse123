...
/* either an error or an interrupt has occured.                    */
    ...
    printf("beforesend\n");
    // If not initial opcode 
    if ( opcode ) { 
        opcode = ((struct ACK_DATA*)(&recvline))->opcode;
    }
    else {
        struct RWRQ * structptr = (struct RWRQ*)&sendline;
        char file[] = "read_file";
        char mode[] = "OCTET";
	      bzero((char *) structptr, sizeof(struct RWRQ)); // zerofill message
        structptr->opcode = 1;     // INITIAL REQUEST (1 OR 2)
        strncpy(structptr->filename,file,strlen(file)); 
        strncpy(structptr->mode,mode,strlen(mode)); 
    }
...
/* using fputs to output it on the terminal.                       */
    ...
    opcode = (((struct ACK_DATA*)(&recvline))->opcode) & MASK;
    printf("Client: opcode #%d\n",opcode);
    ...
    if ( opcode == 1 ) {
        ...
        // DATA
        struct ACK_DATA *rec_ptr = (struct ACK_DATA*)(&recvline);
        printf("Client has received ACK for data packet #%d\n",(rec_ptr->block_num));

        bzero((char*)&sendline,sizeof(sendline));

        struct ACK_DATA *sptr = (struct ACK_DATA*)(&sendline);
        sptr->opcode = 4;
        sptr->block_num = rec_ptr->block_num;
